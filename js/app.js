...
function load(token) {
  ...
  bot.on('ready', () => {
    ...
    if (bot.user.bot) {
      document.getElementById('userCardBot').innerHTML = `БОТ`;
      document.getElementById('userCardBot').style.marginLeft = `8px`;
    } else {
      document.getElementById('userCardBot').innerHTML = `Пользователь`;
...
function command(text) {
  ...
  username = document.createTextNode('Саныч');
  name.appendChild(username);
  name.id = 'messageUsername';
...
let helpMsg = [
  'Это полный список команд. \n',
  '/help - Полный список команд.',
  '/shrug - Добавляет к вашему сообщению ¯\\_(ツ)_/¯',
  '/tableflip - Добавляет к вашему сообщению (╯°□°）╯︵ ┻━┻',
  '/ping - Пинг дискорда.',
  '/server - Получить всю информацию о данном сервере.',
  '/eval - Подсчитать выражение.'
].join('\n')
  ...
  if (selectedChan) {
    ...
    if (text.substring(0,1) == '/') {
      ...
      switch (cmd) {
        ...
        case 'ping':
          command('🏓 | Понг! Мой пинг '+bot.ping+'ms.');
        break;

        case 'server':
          let serverinfo = [
            'Here is some info about '+selectedChan.guild.name+'. \n',
            'Участников - '+selectedChan.guild.memberCount,
            'Каналов - '+selectedChan.guild.channels.size,
            'Ролей - '+selectedChan.guild.roles.size,
            'ID - '+selectedChan.guild.id,
            'Создатель - '+selectedChan.guild.owner.user.tag
          ].join('\n');
          command(serverinfo);
        break;
